import React, { ChangeEvent, useEffect, useState, FC, useContext } from 'react';
...
const EventBody: FC<Props> = ({
  ...
  const validateEventDates = (start: string, end: string, title: string) => {
    ...
    if (!startDate.isValid || !endDate.isValid) {
      ...
      setErrorMsg('Event cannot be more than a month long.');
      onValidate(false);
    } else if (!title.trim()) {
      setErrorMsg('');
      onValidate(false);
  ...
  useEffect(() => {
    const auth = authCtx?.getAuth(); // Use optional chaining
    if (auth) {
